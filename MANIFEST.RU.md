...
[![🇬🇧 Read in English](https://img.shields.io/badge/%F0%9F%87%AC%F0%9F%87%A7-Read%20in%20English-blue)](./MANIFEST.md)
...
### Правило 0: 🎯 Фичи — это пользовательский сценарий

Фича реализует **бизнес-сценарий, с которым может взаимодействовать пользователь**. Внутренние механизмы — cron-задачи, обработчики очередей, фоновые процессы — это не фичи. Они живут в `shared/infra/` и запускаются из `app/`.
...
>
> Держи `index.ts` компактным — экспортируй только то, что реально нужно App-слою (обычно фабрику action и фабрику handler). Не вываливай все внутренние хелперы в баррель.
>
> **Другие языки:** В Java / C# / Go это решается модификаторами доступа (`package-private`, `internal`, неэкспортируемые имена). Баррель-файл не нужен — просто не делай внутренние классы/методы публичными.
...
| Метод | Куда? | Почему |
|---|---|---|
| `findAllActive()` | `dal.ts` | Простой фильтр — `find({ active: true })`. Всё ещё CRUD. |
| `findWithStats()` | `lib/queries.ts` | Агрегация / джойн — выходит за рамки простого `.find()`. |
| `deactivateExpired()` | `lib/commands.ts` | Содержит доменное правило (что считается "просроченным"). |

> [!TIP]
> Если сомневаешься "это CRUD или бизнес-логика?" — если это больше чем один `Model.findOne()` / `find()` с тривиальным фильтром, скорее всего место в `lib/`.
...
```mermaid
graph TD
    START["У меня новый код"] --> Q1{"Чистая утилита?<br/>(datetime, encoding, math)"}
    Q1 -->|Да| SHARED["📁 shared/lib/"]
    Q1 -->|Нет| Q2{"Инфраструктура?<br/>(драйвер БД, логгер, конфиг)"}
    Q2 -->|Да| INFRA["📁 shared/infra/"]
    Q2 -->|Нет| Q3{"Базовый CRUD<br/>для одной сущности?"}
    Q3 -->|Да| DAL["📁 entities/{name}/dal.ts"]
    Q3 -->|Нет| Q4{"Переиспользуемая доменная<br/>логика одной сущности?"}
    Q4 -->|Да| LIB["📁 entities/{name}/lib/"]
    Q4 -->|Нет| Q5{"Сложный запрос<br/>нужный одной фиче?"}
    Q5 -->|Да| FDB["📁 features/{name}/db/"]
    Q5 -->|Нет| ACTION["📁 features/{name}/*.action.ts"]
...
> [!TIP]
> **Эта логика нужна 2+ фичам?** → Entity (`entities/{name}/lib/`).
> **Только одной фиче?** → Оставляй в фиче (`features/{name}/db/` или `features/{name}/lib/`).
...
**Фикс:** вынеси общую логику в Entity или `shared/lib/`. Пример: если `auth` и `notifications` обе отправляют email — `shared/lib/mailer`, а не одна фича импортирует другую.
...
FAA в действии на разных стеках:
