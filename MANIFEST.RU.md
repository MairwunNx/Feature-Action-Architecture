...
```mermaid
graph TD
    START["У меня новый код"] --> Q1{"Чистая утилита?\n(datetime, encoding, math)"}
    Q1 -->|Да| SHARED["📁 shared/lib/"]
    Q1 -->|Нет| Q2{"Инфраструктура?\n(драйвер БД, логгер, конфиг)"}
    Q2 -->|Да| INFRA["📁 shared/infra/"]
    Q2 -->|Нет| Q3{"Базовый CRUD\nдля одной сущности?"}
    Q3 -->|Да| DAL["📁 entities/{name}/dal.ts"]
    Q3 -->|Нет| Q4{"Переиспользуемая доменная\nлогика одной сущности?"}
    Q4 -->|Да| LIB["📁 entities/{name}/lib/"]
    Q4 -->|Нет| Q5{"Сложный запрос\nнужный одной фиче?"}
